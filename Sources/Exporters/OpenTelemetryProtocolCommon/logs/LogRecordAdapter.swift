...
public class LogRecordAdapter {
  ...
  static func toProtoLogRecord(logRecord: ReadableLogRecord) -> Opentelemetry_Proto_Logs_V1_LogRecord {
    ...
    if let observedTimestamp = logRecord.observedTimestamp {
      protoLogRecord.observedTimeUnixNano = UInt64(max(0, observedTimestamp.timeIntervalSince1970.toNanoseconds))
    }

    protoLogRecord.timeUnixNano = UInt64(max(0, logRecord.timestamp.timeIntervalSince1970.toNanoseconds))
