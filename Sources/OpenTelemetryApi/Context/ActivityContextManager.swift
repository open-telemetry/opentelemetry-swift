...
import Foundation
  ...
  private let OS_ACTIVITY_CURRENT = unsafeBitCast(
  ...
  class ActivityContextManager: ImperativeContextManager {
    ...
    var objectScope = NSMapTable<AnyObject, ScopeElement>(
    ...
    func getCurrentContextValue(forKey key: OpenTelemetryContextKeys)
      ...
      let activityIdent = os_activity_get_identifier(
        OS_ACTIVITY_CURRENT, &parentIdent)
    ...
    func setCurrentContextValue(
      forKey key: OpenTelemetryContextKeys, value: AnyObject
    ) {
      var parentIdent: os_activity_id_t = 0
      var activityIdent = os_activity_get_identifier(
        OS_ACTIVITY_CURRENT, &parentIdent)
      rlock.lock()
    ...
    func createActivityContext() -> (
      os_activity_id_t, os_activity_scope_state_s
    ) {
      let dso = UnsafeMutableRawPointer(mutating: #dsohandle)
      let activity = _os_activity_create(
        dso, "ActivityContext", OS_ACTIVITY_CURRENT, OS_ACTIVITY_FLAG_DEFAULT)
