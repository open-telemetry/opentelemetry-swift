...
class RawHistogramMetricSdkBase<T>: RawHistogramMetric {
  ...
  public private(set) var boundInstruments = [
    LabelSet: BoundRawHistogramMetricSdkBase<T>
  ]()
  ...
  internal func bind(labelset: LabelSet, isShortLived: Bool)
    ...
    bindUnbindLock.withLockVoid {
      ...
      if boundInstrument == nil {
        let status =
          isShortLived ? RecordStatus.updatePending : RecordStatus.bound
  ...
  func createMetric(recordStatus: RecordStatus)
    -> BoundRawHistogramMetricSdkBase<T> {
    // noop
    fatalError()
  }
