...
public class HttpTestServer: @unchecked Sendable {
    ...
    private func handleClient(socket clientSocket: Int32) {
        ...
        while true {
            ...
            if totalData.range(of: Data([0x0D, 0x0A, 0x0D, 0x0A])) != nil {
                break
            }
        }
        ...
        _ = parts[0]  // method - unused but needed for parsing
        let path = parts[1]
...
public struct HTTPHeaders {
    ...
    public func contains(where predicate: (HTTPHeader) -> Bool) -> Bool {
        for (name, value) in headers where predicate(HTTPHeader(name: name, value: value)) {
            return true
        }
        return false
    }
