...
struct UploadPerformanceMock: UploadPerformancePreset {
  ...
  static let veryQuick = UploadPerformanceMock(initialUploadDelay: 0.05,
...
extension DataFormat {
  ...
  static func mockWith(prefix: String = .mockAny(),
                       suffix: String = .mockAny(),
                       separator: String = .mockAny()) -> DataFormat {
    return DataFormat(prefix: prefix,
                      suffix: suffix,
                      separator: separator)
  }
}
...
extension RequestBuilder: AnyMockable {
  ...
  static func mockWith(url: URL = .mockAny(),
                       queryItems: [QueryItem] = [],
                       headers: [HTTPHeader] = []) -> RequestBuilder {
...
extension Device {
  ...
  static func mockWith(model: String = .mockAny(),
                       osName: String = .mockAny(),
                       osVersion: String = .mockAny()) -> Device {
...
extension DataUploadStatus: RandomMockable {
  ...
  static func mockWith(needsRetry: Bool = .mockAny(),
                       accepted: Bool = true) -> DataUploadStatus {
